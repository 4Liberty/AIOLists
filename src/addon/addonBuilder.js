...
async function createAddon(userConfig) {
  ...
  let hasValidSearchSources = userSearchSources.includes('trakt') || (userSearchSources.includes('tmdb') && (userConfig.tmdbBearerToken || require('../config').TMDB_BEARER_TOKEN));
  ...
  builder.defineCatalogHandler(async ({ type, id, extra }) => {
    ...
    if ((id.includes('_search')) && searchQuery) {      
      ...
      try {
        ...
        if (id === 'aiolists_merged_search') {
          ...
          const userSearchSources = userConfig.searchSources || [];
          let sources = userSearchSources.filter(s => s === 'trakt' || (s === 'tmdb' && (userConfig.tmdbBearerToken || userConfig.tmdbSessionId)));
    ...
    const metadataConfig = { metadataSource: userConfig.metadataSource || 'tmdb', tmdbLanguage: userConfig.tmdbLanguage || 'en-US' };
