...
async function createAddon(userConfig) {
  ...
  const hasTmdbAccessForGenres = !!(userConfig.tmdbSessionId || userConfig.tmdbAccountId || userConfig.tmdbBearerToken || require('../config').TMDB_BEARER_TOKEN);
  if (hasTmdbAccessForGenres) {
    try {
      ...
      const tmdbGenres = await fetchTmdbGenres(userConfig.tmdbLanguage || 'en-US', userConfig.tmdbBearerToken || require('../config').TMDB_BEARER_TOKEN);
